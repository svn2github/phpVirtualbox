...
class vboxconnector {
	...
	public function remote_machineSave($args) {
		...
		foreach($args['storageControllers'] as $sc) {
			...
			foreach($sc['mediumAttachments'] as $ma) {
				...
				if (($ma['type'] == 'HardDisk' || $ma['type'] == 'DVD') && $sc['bus'] != 'IDE') {
	...
	public function remote_mediumChangeEncryption($args) {
	    ...
	    // "An empty password and password ID will result in the medium being encrypted with the current password."
	    if(!@$args['password']) {
	    ...
	    $skipPWcheck = false;
	    try {
	        $m->getEncryptionSettings();
	    } catch (Exception $null) {
	        $skipPWcheck = true;
	    }

	    // Check password before we start this operation
	    if(!$skipPWcheck)
	        $m->checkEncryptionPassword($args['old_password']);
